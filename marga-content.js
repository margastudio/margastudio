(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      /* El wrapper de "Product designer" solo tenía un hijo (el <p> del
         texto); se lo hace flex para poder poner el selector de idioma
         al lado, en la misma línea, en vez de superpuesto como overlay. */
      .framer-1ndf9in {
  ...
  function startObserver() {
    ...
    // En la carga normal de una pagina esto nunca pasa (el script se
    // difiere hasta DOMContentLoaded mas abajo), pero en algunos entornos
    // de preview/sandbox el documento puede reportarse como "listo" antes
    // de que <body> exista de verdad. observe() tira un TypeError si el
    // nodo es null, asi que mejor esperar en vez de asumir.
    if (!document.body) {
  ...
  // No alcanza con mirar document.readyState: en algunos entornos de
  // preview/sandbox puede reportar 'interactive' o 'complete' aunque
  // document.body todavia no exista como Node real, y varias funciones de
  // aca arriba (collectSources, startObserver) lo necesitan si o si. Se
  // chequea el body directamente en vez de confiar solo en readyState.
  if (document.body) {
    init();
  } else {
